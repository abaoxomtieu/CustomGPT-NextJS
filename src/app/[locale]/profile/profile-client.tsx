  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className={`absolute top-1/4 left-1/4 bg-blue-primary/5 rounded-full blur-3xl animate-pulse ${
            isMobile ? "w-32 h-32" : "w-64 h-64"
          }`}
        ></div>
        <div
          className={`absolute bottom-1/4 right-1/4 bg-blue-60/5 rounded-full blur-3xl animate-pulse delay-1000 ${
      ...
      <div
        className={`container mx-auto relative z-10 flex flex-col ${
          isMobile
            ? "max-w-full px-2 py-4"
            : "max-w-4xl py-8 items-center justify-center min-h-screen"
        }`}
      >
        {/* Hero Header Section */}


        <div className={`w-full mb-5 ${isMobile ? "" : "max-w-2xl"}`}>
          <Card className="glass-card border-blue-60/20 bg-card/80 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300 animate-fade-in delay-100">
            ...
            `}</style>
            <CardHeader
              className={`border-b border-blue-60/10 space-y-2 ${
                isMobile ? "p-4" : ""
              }`}
            >
              <div
              ...
              >
                <div
                  className={`rounded-lg bg-blue-primary/10 text-blue-primary ${
                    isMobile ? "p-1.5" : "p-2"
                  }`}
                >
            ...
            <CardContent className={`space-y-8 ${isMobile ? "p-4" : "p-6"}`}>
              <Form {...form}>
                ...
                >
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          <FormControl>
                            <SelectTrigger
                              className={`bg-background/80 border-border/50 text-foreground ${
                                isMobile ? "h-11" : ""
                              }`}
                            >
                              <SelectValue
                                placeholder={t("form.major.placeholder")}
                              />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                  ...
                  <Button
                    type="submit"
                    className={`w-full bg-gradient-to-r from-blue-primary to-blue-active hover:from-blue-active hover:to-blue-primary shadow-md hover:shadow-lg transition-all duration-300 touch-manipulation ${
                      isMobile ? "h-11 text-base" : ""
                    }`}
                    disabled={loading}
                  >
              ...
              <div className={`${isMobile ? "space-y-3" : "space-y-4"}`}>
                <div
                  className={`flex ${
                    isMobile
                      ? "flex-col gap-2"
                      : "flex-col sm:flex-row sm:items-center sm:justify-between gap-2"
                  }`}
                >
                  <Label
                    className={`text-card-foreground ${
                  ...
                  {isApiKeyValid ? (
                    ...
                    >
                      <CheckCircle
                        className={`mr-1 ${isMobile ? "w-4 h-4" : "w-4 h-4"}`}
                      />
                      <span className="text-sm">
                        {t("gemini.status.valid")}
                      </span>
                    </div>
                  ) : (
                    <div
                ...
                <Form {...geminiForm}>
                  <form
                    onChange={handleGeminiFormChange}
                    className={`${isMobile ? "space-y-3" : "space-y-4"}`}
                  >
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          >
                            <FormControl>
                              <SelectTrigger
                                className={`bg-card/50 border-blue-60/30 hover:border-blue-primary/50 focus:border-blue-primary text-foreground transition-colors duration-200 ${
                                  isMobile ? "h-11" : ""
                                }`}
                              >
                                <SelectValue
                                  placeholder={t(
                                    "gemini.form.model.placeholder"
                                  )}
                                />
                              </SelectTrigger>
                            </FormControl>
                    ...
                    >
                      <Button
                        ...
                        className={`text-foreground touch-manipulation ${
                          isMobile ? "w-full h-11 text-base" : "flex-1"
                        }`}
                      ...
                      <Button
                        type="button"
                        className={`bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-600 shadow-md hover:shadow-lg transition-all duration-300 touch-manipulation ${
                          isMobile ? "w-full h-11 text-base" : "flex-1"
                        }`}
